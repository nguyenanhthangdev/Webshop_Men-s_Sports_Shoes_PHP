...
function orderGetTotalForTheDay($data = array()) 
{
		date_default_timezone_set("Asia/Ho_Chi_Minh");
		$time = date('Y-m-d');
		$sql = "SELECT COUNT(*) AS total FROM `order` WHERE `date_added` LIKE '$time%'";
		return db_one($sql);
}// end function
...
function orderGetTotalSalesForTheDay()
{
	date_default_timezone_set("Asia/Ho_Chi_Minh");
	$time = date('Y-m-d');
...
function orderGetTotalSalesWithFormatForTheDay()
{
	
	$totalSales = (int)orderGetTotalSalesForTheDay();

	if (0 <= $totalSales && $totalSales < 1000)
		return (int)($totalSales).' đồng'; //' triệu ₫';
...
function totalNumberOfCustomersForTheDay(){
	date_default_timezone_set("Asia/Ho_Chi_Minh");
	$time = date('Y-m-d');

	$sql = "
		SELECT COUNT(*) AS 'amountCustomer'
		FROM `customer` WHERE `date_added` LIKE '$time%'
	";

	return (int)db_one($sql);
}

function totalNumberOfCustomersForTheMonth($time){
	$sql = "
		SELECT
			SUM(total)
		FROM `order` WHERE `date_added` LIKE '$time%';
...
function dayMonthYear($day, $month, $years){
    if((int)$day == 1){
        if((int)$month == 1){
            ...
            else if((int)$monthNew == 5||(int)$monthNew == 7||(int)$monthNew == 8||(int)$monthNew == 10||(int)$monthNew == 12){
                $dayNew = 31;
            }
            else{
                $dayNew = 30;
