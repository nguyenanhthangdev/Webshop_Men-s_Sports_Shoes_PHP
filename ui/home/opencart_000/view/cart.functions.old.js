...
function addToCart(product_id, quantity, size) 
{ 
	$.ajax({
		url: '/cart-add.php',
		type: 'post',
		data: 'size=' + size + '&product_id=' + product_id + '&quantity=' + (typeof(quantity) != 'undefined' ? quantity : 1),
		dataType: 'json',
		beforeSend: function() {
...
function cartAdd(product_id, quantity, size) { addToCart(product_id, quantity, size); }
...
function removeFromCart(key, size) 
{ 
		$.ajax({
			url: '/cart-remove.php', 
			type: 'post',
			data: 'size=' + size + '&key=' + key,
			dataType: 'json',
			beforeSend: function() {
				$('#cart > button').button('loading');
			},
...
function cartRemove(key,size) {removeFromCart(key,size);}
