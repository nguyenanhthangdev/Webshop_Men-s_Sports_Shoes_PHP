...
<!-- *************************************************************************************************** -->
               <div class="form-group">
	                <label class="control-label" for="input-sale">Giảm Giá</label>
	                <select name="filter_sale" id="input-sale" class="form-control">
	                  <option value="*">--Không chọn--</option>
	                  <?php if ($filter_sale) { ?>
	                  ...
	                  <?php } ?>
	                  <?php if (!$filter_sale && !is_null($filter_sale)) { ?>
	                  <option value="0" selected="selected">Không giảm giá</option>
	                  <?php } else { ?>
				  ...
				  <div class="form-group">
	                <label class="control-label" for="input-new">Sản Phẩm Mới</label>
	                <select name="filter_new" id="input-new" class="form-control">
	                  <option value="*">--Không chọn--</option>
	                  <?php if ($filter_new) { ?>
	                  ...
	                  <?php } ?>
	                  <?php if (!$filter_new && !is_null($filter_new)) { ?>
	                  <option value="0" selected="selected">Không Phải Sản Phẩm Mới</option>
	                  <?php } else { ?>
	                  <option value="0">Không Phải Sản Phẩm Mới</option>
	                  <?php } ?>
	                </select>
	              </div>
<!-- *************************************************************************************************** -->
            ...
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <td style="width: 100px;" class="text-center"><?php echo 'Ảnh'; ?></td>
                  <td class="text-left">
                  ...
                  <td class="text-left">
                    <a href="<?php echo $sort_price; ?>" class="<?php if ($sort == 'p.price') {echo strtolower($order);} ?>"><?php echo 'Size-Giá'; ?></a>
                  </td>
				  <td class="text-left">
                    <a>Màu Sắc</a>
                  </td>
...
<!-- *************************************************************************************************************** -->
					<td class="text-left">
						<a href="<?php echo $sort_new; ?>" class="<?php if ($sort == 'p.new') echo strtolower($order); ?>">Sản Phẩm Mới</a>
					</td>
				  ...
				  <td class="text-left">
                    <a href="<?php echo $sort_amount; ?>" class="<?php if ($sort == 'p.amount') echo strtolower($amount); ?>">Size - Số lượng</a>
                  </td>
<!-- *************************************************************************************************************** -->
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-center"><?php if ($product['image']) { ?>
                    <img src="<?php echo $product['image']; ?>" alt="<?php echo $product['name']; ?>" width="70px" height="70px" />
                    <?php } else { ?>
                  ...
                  <td class="text-left">
					<?php if($product['size_or_nosize'] == 1) { ?>
				  		<?php foreach($sizes as $size) { ?>
							<?php if($size['product_id'] == $product['product_id']) { ?>
								<?php $priceVND = currency_format($size['priceSize']); ?>
								<p style="font-size: 11px;" class="badge badge-success"><?php echo $size['size'] ." - ". $priceVND; ?></p>
							<?php } ?>
						<?php } ?>
					<?php } ?>
					<?php if($product['size_or_nosize'] == 0) { ?>
						<?php foreach($amount_price as $item) { ?>
							<?php if($item['product_id'] == $product['product_id']) { ?>
								<?php $priceVND = currency_format($item['price']); ?>
								<p style="font-size: 11px;" class="badge badge-danger"><?php echo $priceVND; ?></p>
							<?php } ?>
						<?php } ?>
					<?php } ?>
				  </td>
				  <td class="text-left"><p style="border: 2px solid black; width:50px; height: 20px; background-color: <?php echo $product['color']; ?>;"></p></td>
...
<!-- *************************************************************************************************************** -->
				  ...
				  <td class="text-left">
						<?php foreach($sizes as $size) { ?>
							<?php if($size['product_id'] == $product['product_id']) { ?>
								<?php if($size['amount'] == 0) { ?>
									<p class="badge badge-danger"><?php echo $size['size'] ."-". $size['amount']; ?></p>
								<?php } ?>
								<?php if($size['amount'] != 0) { ?>
									<p class="badge badge-success"><?php echo $size['size'] ."-". $size['amount']; ?></p>
								<?php } ?>
							<?php } ?>
						<?php } ?>
				  </td>
<!-- *************************************************************************************************************** -->
                ...
                <?php } ?>
                <tr>
                  <td class="text-center" colspan="13"><?php echo "Tìm thấy {$product_total} sản phẩm"; ?></td>
                </tr>
                <?php } else { ?>
                <tr>
                  <td class="text-center" colspan="13"><?php echo 'Không tìm thấy kết quả nào.'; ?></td>
                </tr>
                <?php } ?>
              </tbody>
            </table>
...
$('#button-filter').on('click', function() {
  ...
  //***************************************************************************************************** */
  // Tính giảm giá (có/không) trên thẻ select
	var filter_sale = $('select[name=\'filter_sale\']').val();

  // Nếu người dùng không chọn thẻ select thì thôi, còn nếu chọn Giảm giá/Không giảm giá
	// thì gắn vào url
	if (filter_sale != '*') 
	{
	...
	// Tính còn hàng (có/không) trên thẻ select
	var filter_amount = $('select[name=\'filter_amount\']').val();

  // Nếu người dùng không chọn thẻ select thì thôi, còn nếu chọn còn hàng/hết hàng
	// thì gắn vào url
	if (filter_amount != '*') 
	{
	  ...
	  // Tính new (có/không) trên thẻ select
	  var filter_new = $('select[name=\'filter_new\']').val();

	// Nếu người dùng không chọn thẻ select thì thôi, còn nếu chọn Nổi bật/Không nổi bật
	// thì gắn vào url
	if (filter_new != '*') 
	{
		url += '&filter_new=' + encodeURIComponent(filter_new);
	}
  //***************************************************************************************************** */
