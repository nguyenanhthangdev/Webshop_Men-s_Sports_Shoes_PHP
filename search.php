...
if (isset($_REQUEST['search']) || isset($_REQUEST['tag'])) 
{
	...
	foreach ($results as $result) 
	{
		...
		if($result['size_or_nosize'] == 1){
			$rsPrice = getPriceOfProductWithSize($result['product_id']);
			$x = 0;
			$rowPrice = [];
			foreach($rsPrice as $item){
				$rowPrice[$x] = $item['priceSize'];
				$x++;
			}
		}
		else{
			$rowPrice = getPriceOfProductWithNoSize($result['product_id']);
		}
